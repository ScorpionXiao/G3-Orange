...
// ── District centroid (avg GPS per dataset district letter) — for mapping GeoJSON polygons to A–J
$sql_district_centroids = "
    SELECT
        TRIM(district) AS district_code,
        AVG(CAST(lat AS DECIMAL(10, 7))) AS alat,
        AVG(CAST(lng AS DECIMAL(10, 7))) AS alng,
        COUNT(*) AS n
    FROM ca_san_francisco_50k
    WHERE lat IS NOT NULL AND lat != ''
      AND lng IS NOT NULL AND lng != ''
...
// ── Route and respond ─────────────────────────────────────────────────────
switch ($task) {
    case 'district_centroids':
        echo json_encode(["task" => "district_centroids", "data" => runQuery($mysql, $sql_district_centroids)]);
        break;
    case 'geo':
    ...
    default: // 'all'
        echo json_encode([
            ...
            "district_centroids" => runQuery($mysql, $sql_district_centroids),
        ]);
        break;
}
